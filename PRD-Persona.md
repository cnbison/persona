...
|核心目标|以“作者Persona”为核心资产，构建可解释、可验证、可复用的Persona体系；文档解析、提纲生成、对话生成作为Persona的实践与验证路径；后续扩展到多音色可选的播客音频生成|
...
- **对用户**：获得高一致性、可解释、可追溯的作者Persona，用于理解与应用原著思想。
- **对产品**：Persona作为核心资产，沉淀可复用的模型/配置与可验证流程。
- **对运营**：以Persona为中心组织内容与场景扩展，降低单场景开发成本。
...
内容传播包含“说者Persona（作者）”与“受众Persona（接受者）”两部分。受众Persona用于指导表达方式与难度控制，同一内容在不同受众画像下应表现为不同的讲解方式、术语密度与案例选择。
...
|需求点|优先级|需求描述|验收标准|
|---|---|---|---|
|System Prompt生成|P0|自动生成包含六维度、边界与风格规则的Persona指令集。|1. 覆盖六维度；2. 指令清晰无歧义。|
|观点边界约束|P0|设定不可突破观点清单与推演标注规则。|1. 无违背核心主张；2. 推演标注清晰。|
|原文锚定机制|P0|Persona输出需匹配证据库，不匹配则触发校验与修正。|1. 观点匹配率100%；2. 修正响应≤1秒。|
...
```
著作文件上传 → 格式解析与清洗 → 内容结构化提取 → Persona证据库 → Persona画像构建（作者Persona + 受众Persona） → Persona执行配置 → 受众适配输出 → 审核与一致性修正
```
